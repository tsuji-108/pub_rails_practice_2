...
puts "Seeded #{Discussion.where(
  user_id: discussions.map { |c| c[:user_id] },
  board_id: discussions.map { |d| d[:board_id] },
  title: discussions.map { |d| d[:title] },
  description: discussions.map { |d| d[:description] },
  ).count} discussios."
